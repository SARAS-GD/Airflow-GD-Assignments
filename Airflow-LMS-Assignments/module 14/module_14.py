...
with DAG(
    ...
    # Dummy task if the table already exists
    dummy_task = BashOperator(
        task_id='dummy_task',
        bash_command='echo "Table already exists"'
    )
    
    # Task to insert a row into the table
    insert_row = PostgresOperator(
        ...
        trigger_rule=TriggerRule.NONE_FAILED_MIN_ONE_SUCCESS  #  even if one branch is skipped it will run

    )
    
    # Task to query the table and get row count using custom operator
